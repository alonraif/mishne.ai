...
Four properties here, and each is a way that was allowed to be true:

* the rows appear at all, in frames rather than milliseconds;
...
def _ingest() -> FakeIngest:
    return FakeIngest(
        beats=[
            FakeBeat("b1", 0, 2_000, 6_000, "The harbour is being closed."),
            FakeBeat("b2", 1, 6_400, 20_000, "A long answer with room inside it."),
        ],
...
def test_the_beats_arrive_in_frames_against_the_assets_own_timecode(
    ...
    with _session(owner) as s:
        writes.record_asset(s, ORG, _ingest(), ingest_version=7)
        s.commit()
...
def test_a_re_ingest_does_not_unname_a_speaker_a_person_named(owner, asset_and_job):
    ...
    with _session(owner) as s:
        writes.record_asset(s, ORG, _ingest(), ingest_version=8)
        s.commit()
...
def test_a_beat_carved_into_two_selected_spans_is_two_clips(owner, asset_and_job):
    ...
    candidates = [
        FakeBeat("b1", 0, 2_000, 6_000, "…", rationale="Sharp."),
        FakeBeat("b2#a", 1, 6_400, 11_000, "…", parent_id="b2", rationale="Good half"),
        FakeBeat("b2#b", 2, 15_000, 20_000, "…", parent_id="b2", rationale="Better half"),
    ]
    cuts = [
        FakeCut("b1", "b1", ASSET, START_TC + 50, START_TC + 150),
        FakeCut("b2#a", "b2", ASSET, START_TC + 160, START_TC + 275),
        FakeCut("b2#b", "b2", ASSET, START_TC + 375, START_TC + 500),
    ]
    ...
    with _session(owner) as s:
        ...
        writes.record_job_view(
            ...
            scores={"b1": 90.0, "b2#a": 40.0, "b2#b": 71.0},
            cuts=cuts,
        )
        s.commit()
    ...
    with owner.connect() as conn:
        selections = conn.execute(
            sa.text(
                "SELECT beat_id, order_idx FROM selections WHERE org_id = :o "
                "ORDER BY order_idx"
            ),
            {"o": ORG},
        ).all()
    assert [r.beat_id for r in selections] == ["b1", "b2", "b2"]
...
def test_a_beat_keeps_its_best_candidates_score(owner, asset_and_job):
    ...
    with _session(owner) as s:
        writes.record_asset(s, ORG, _ingest(), ingest_version=7)
        writes.record_job_view(
            s, ORG, JOB, candidates=candidates,
            scores={"b2#a": 40.0, "b2#b": 71.0}, cuts=[],
        )
        s.commit()
...
def test_re_running_a_job_replaces_its_opinion_rather_than_doubling_it(
    ...
    cuts = [FakeCut("b1", "b1", ASSET, START_TC + 50, START_TC + 150)]
    with _session(owner) as s:
        writes.record_asset(s, ORG, _ingest(), ingest_version=7)
        for _ in range(2):
            writes.record_job_view(
                s, ORG, JOB,
                candidates=[FakeBeat("b1", 0, 2_000, 6_000, "…")],
                scores={"b1": 90.0}, cuts=cuts,
            )
        s.commit()
...
def test_the_api_reads_back_one_beat_per_beat(owner, asset_and_job):
    ...
    candidates = [
        FakeBeat("b1", 0, 2_000, 6_000, "…"),
        FakeBeat("b2#a", 1, 6_400, 11_000, "…", parent_id="b2"),
        FakeBeat("b2#b", 2, 15_000, 20_000, "…", parent_id="b2"),
    ]
    cuts = [
        FakeCut("b2#a", "b2", ASSET, START_TC + 160, START_TC + 275),
        FakeCut("b2#b", "b2", ASSET, START_TC + 375, START_TC + 500),
    ...
    with _session(owner) as s:
        ...
        writes.record_job_view(
            s, ORG, JOB, candidates=candidates,
            scores={"b1": 10.0, "b2#a": 40.0, "b2#b": 71.0}, cuts=cuts,
        )
        s.commit()
    ...
    assert [b.id for b in transcript.beats] == ["b1", "b2"]
    cut_beat = next(b for b in transcript.beats if b.id == "b2")
    ...
    assert next(b for b in transcript.beats if b.id == "b1").used is False
...
def _beats(owner):
    with owner.connect() as conn:
        return conn.execute(
            sa.text(
                "SELECT id, start_frames, end_frames, speaker FROM beats "
